...
\begin{frame}
	\frametitle {Runtime Approximation III}	
		\begin{itemize}
			\pause
			\item{The total sum of all influence factors is 1}
			\pause			
...
\begin{minipage}[t]{0.48\linewidth}
		\begin{itemize}
			\item{Check whether the task is 'suited' as value of a 													surrounding grid point}
			\begin{itemize}
				\item{grid points 'always' point to the closet task already inserted}
				\item{note that inserting a point only adapts the 														closest grid points}
			\end{itemize}
			\item{Adapt surrounding grid points, if necessary}
		\end{itemize}
		
		\begin{itemize}
			\item{'Hard insert': check \textbf{all} grid points and eventually adapt them}
		\end{itemize}
	\end{minipage}\hfill
	\begin{minipage}[t]{0.48\linewidth}
		\vspace{-\ht\strutbox}
