	...
	\subsubsection{Command line parameters}
		...
		\begin{itemize}	
			...
			\item OPTIONS:
				...
				\subitem -m: choose between multiple-queues scheduling (multi-queue) and single-queue scheduling (single-queue) (e.g. -m multi-queue or -m single-queue)
				
				\subitem -a: choose between the provided scheduling algorithm such as task-stealing, FIFO, LIFO, SJF, LSF and statistic based scheduling (e.g. -a fifo)
				\subitem Default is single-queue with FIFO scheduling (task-stealing and single-queue can not be combined)
					
			\item RUN ARGUMENTS:
				\subitem The run arguments will be passed to the scientific code to initialized the code
		\end{itemize}
