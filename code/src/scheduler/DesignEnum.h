...
enum DesignEnum {
    /**
     * The task stealing design uses peer to peer communication.
     * One process is used as database server. The other processes are independent schedulers with distinct queues.
     * The scheduler tries to pop the next task from his own queue. If the queue is empty the scheduler steals a task
     * from another scheduler using MPI one site communication.
     */
    TASK_STEALING,
     ...
     * The master has a scheduling queue and distributes the scientific tasks to free workers.
