...
#include <vector>
/**
 * This is a custom comparison class for the std::priority_queue. This class is used to order the priority queue.
 ...
 * The LJF class uses the std::priority_queue priority queue to store the tasks.
 *
 * @author Fabio Broghammer
 * @version 1.0
 */
...
public:
    /**
     * Creates a new SJF scheduling strategy.
     */
    LJF();
     ...
     *
     * All tasks will get a default rumtime value (defined in DEFAULT_RUNTIME) by changing from non-statistically based strategies (FIFO, LIFO) to statistically based strategies. The order of the old queue might change.
     * @param new_strategy
     */
